...
async function run() {
  try {
    ...
    const menuCallection = client.db("restaurant").collection("menu");
    const reviewsCallection = client.db("restaurant").collection("reviews");
    app.get("/menu", async (req, res) => {
      const result = await menuCallection.find().toArray();
      res.send(result);
    });
